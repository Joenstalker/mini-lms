        ...
        <div class="card glass-card hover:shadow-2xl transition-all duration-500 hover:border-primary/40 group/card">
            <div class="card-body space-y-4">
                ...
                <div class="card-actions justify-end gap-2 mt-2">
                    <button @click="fetchAuthorDetails('{{ route('authors.show', $author) }}')" class="btn btn-sm btn-ghost bg-info/10 text-info hover:bg-info hover:text-info-content transition-all duration-300 rounded-xl border border-info/20 shadow-lg shadow-info/10 group" title="View Profile">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                        </svg>
                    </button>
                    <button @click="openEditModal({{ Js::from([
                        'id' => $author->id,
                        'name' => $author->name,
                        'bio' => $author->bio,
                    ...
                    <button type="button" 
                        @click="confirmDelete('{{ route('authors.destroy', $author) }}', '{{ csrf_token() }}')"
                        class="btn btn-sm btn-ghost bg-error/10 text-error hover:bg-error hover:text-error-content transition-all duration-300 rounded-xl border border-error/20 shadow-lg shadow-error/10" 
                        title="Delete Author">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
