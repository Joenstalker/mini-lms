            ...
            <div class="card-body space-y-4">
                ...
                <div class="card-actions justify-end gap-2 mt-2">
                    ...
                    <form method="POST" action="{{ route('authors.destroy', $author) }}" class="inline delete-form">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn btn-sm btn-ghost bg-error/10 text-error hover:bg-error hover:text-error-content transition-all duration-300 rounded-xl border border-error/20 shadow-lg shadow-error/10 confirm-delete" title="Delete Author">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
