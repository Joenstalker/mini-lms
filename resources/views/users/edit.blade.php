    ...
    <div class="space-y-6 max-w-4xl mx-auto">
        ...
        <div class="glass text-white rounded-2xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border border-white/10 shadow-xl">
            <div class="flex items-center gap-6">
                <div class="avatar shadow-2xl border-4 border-white/10 rounded-[2rem] overflow-hidden w-20 h-20 shrink-0">
                    @if($user->profile_image)
                        <img src="{{ $user->profile_image }}" class="w-full h-full object-cover">
                    @else
                        <div class="bg-gradient-to-br from-primary to-primary-focus text-white w-full h-full flex items-center justify-center font-black text-3xl">
                            {{ strtoupper(substr($user->name, 0, 1)) }}
                        </div>
                    @endif
                </div>
                <div>
                    <h1 class="text-4xl font-bold italic tracking-tighter">EDIT <span class="text-primary">ADMIN</span></h1>
                    <p class="text-lg text-white/60 mt-1 font-medium italic">Modifying: {{ $user->name }}</p>
                </div>
            </div>
        ...
        <div class="glass-card rounded-[2.5rem] shadow-2xl border border-white/10 overflow-hidden relative">
            ...
            <form action="{{ route('users.update', $user) }}" method="POST" class="p-10 space-y-8 relative z-10">
                @csrf
                @method('PATCH')
                ...
                <div class="bg-white/5 rounded-3xl p-8 border border-white/5 space-y-6">
                    <div class="flex items-center gap-4 mb-2">
                        <div class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                        </div>
                        <div>
                            <h4 class="text-sm font-black uppercase tracking-widest text-white">Security Update</h4>
                            <p class="text-xs text-white/40">Leave password fields blank if you don't want to change it.</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8" x-data="{ showPw: false, showConfirm: false }">
                        ...
                        <div class="form-control">
                            <label class="label">
                                <span class="label-text font-black text-[10px] uppercase tracking-[0.2em] text-white/40">New Password</span>
                            </label>
                            <div class="relative">
                        ...
                        <div class="form-control">
                            <label class="label">
                                <span class="label-text font-black text-[10px] uppercase tracking-[0.2em] text-white/40">Confirm New Password</span>
                            </label>
                            <div class="relative">
